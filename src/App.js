...
export default class App extends React.Component {
	...
	updateFromHash() {
		...
		if (!this.checkBoardID(potentialBoardID, { path, load: true })) {
			// Default board
			this.checkBoardID('G1QmhGhL', { load: true })
		}
	}
	...
	checkBoardID(input, { path = '', load = false } = {}) {
		input = input.trim()
	...
	render() {
		const { boardID, boardError } = this.state
		...
		return (
			<main>
				...
				<section style={ iframeSectionStyle }>
